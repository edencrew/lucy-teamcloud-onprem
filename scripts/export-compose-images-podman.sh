...
BUILD_IMAGE="localhost/lucy-teamcloud-onprem-init-secrets:offline"
...
DESCRIPTION
  Export images required by compose.podman.yml and
  compose.podman.init-secrets.yml into a tar.gz archive.

USAGE
  ./scripts/export-compose-images-podman.sh [OPTIONS]
...
ENVIRONMENT
  ...
  OUTPUT_NAME
      ...
      lucy-teamcloud-onprem-podman-images-<platform>.tar.gz.
EOF
}
...
OUTPUT_NAME="${OUTPUT_NAME:-lucy-teamcloud-onprem-podman-images-$PLATFORM_SAFE.tar.gz}"
ARCHIVE_PATH="$OUTPUT_DIR/$OUTPUT_NAME"
COMPOSE_ARGS=(
  --env-file "$ROOT_DIR/.env.example"
  -f "$ROOT_DIR/compose.podman.yml"
  -f "$ROOT_DIR/compose.podman.init-secrets.yml"
)

mkdir -p "$OUTPUT_DIR"

log "Project root: $ROOT_DIR"
log "Compose files: compose.podman.yml compose.podman.init-secrets.yml"
