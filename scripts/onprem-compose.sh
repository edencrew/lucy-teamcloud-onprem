...
DESCRIPTION
  ...
  The script auto-detects the same compose files as preflight-onprem.sh:
    1. Base compose file
...
COMMANDS
  check
      Run scripts/preflight-onprem.sh.
...
write_image_override_from_manifest() {
  ...
  {
    printf '# Generated by scripts/onprem-compose.sh replace-images.\n'
...
main() {
  case "${1:-}" in
    -h|--help)
      ...
      ;;
    -v|--version)
      echo "onprem-compose.sh $SCRIPT_VERSION"
      exit 0
      ;;
    "")
      show_help
