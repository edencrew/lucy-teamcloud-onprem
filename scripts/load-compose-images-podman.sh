...
DESCRIPTION
  Verify and load a Docker image archive into Podman.
...
validate_image_list() {
  ...
  grep -Fxq "localhost/lucy-teamcloud-onprem-init-secrets:offline" "$image_list" \
    || die "This is not a Podman archive image list: missing localhost/lucy-teamcloud-onprem-init-secrets:offline"

  if grep -Fxq "lucy-teamcloud-onprem-init-secrets:offline" "$image_list"; then
    die "This looks like a Docker archive. Use ./scripts/load-compose-images-docker.sh instead."
  fi
}
