...
RUNTIME="podman"
ARCHIVE_PREFIX="lucy-teamcloud-dmz-podman-images"
...
ENVIRONMENT
  ...
  IMAGES_DIR
      Directory containing one image archive. Defaults to <dmz-root>/images.
...
find_single_archive() {
  ...
  for f in "$dir"/$ARCHIVE_PREFIX-*.tar.gz "$dir"/$ARCHIVE_PREFIX-*.tgz "$dir"/$ARCHIVE_PREFIX-*.tar; do
    [ -f "$f" ] || continue
    matches+=("$f")
  done

  if [ "${#matches[@]}" -eq 0 ]; then
    die "No Podman DMZ image archive found in: $dir"
  fi
  if [ "${#matches[@]}" -gt 1 ]; then
    printf '%s\n' "${matches[@]}" >&2
    die "Multiple Podman DMZ archives found. Pass one explicitly."
  fi
...
validate_image_list() {
  ...
  grep -Fxq "docker.io/library/nginx:1.26-alpine" "$image_list" \
    || die "This is not a DMZ image list: missing docker.io/library/nginx:1.26-alpine"
}
...
verify_loaded_image() {
  ...
  alias="$(podman_loaded_alias "$image")"
  if [ -n "$alias" ] && "$RUNTIME" image inspect "$alias" >/dev/null 2>&1; then
    log "Tagging Podman-loaded Docker Hub alias: $alias -> $image"
    "$RUNTIME" tag "$alias" "$image"
    return 0
  fi
...
if [ "$SKIP_IMAGE_VERIFY" -eq 0 ]; then
  ...
  while IFS= read -r image; do
    [ -n "$image" ] || continue
    verify_loaded_image "$image" || die "Image not found after load: $image"
