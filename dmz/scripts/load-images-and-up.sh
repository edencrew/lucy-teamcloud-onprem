...
# DMZ standalone image loader and runner.
# Verifies and loads a DMZ image archive, then starts the DMZ compose stack.
...
USAGE
  ./scripts/load-images-and-up.sh [OPTIONS] [ARCHIVE_PATH]
...
EXAMPLES
  ...
  Skip checksum verification:
    ./scripts/load-images-and-up.sh --skip-checksum ./images/dmz-images.tar.gz
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      -h|--help)
        ...
        ;;
      -v|--version)
        echo "load-images-and-up.sh $SCRIPT_VERSION"
        exit 0
        ;;
      --skip-checksum)
        SKIP_CHECKSUM=1
...
main() {
  ...
  log "Loading Docker image archive..."
